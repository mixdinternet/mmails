...
@section('btn-insert')
    @if((!checkRule('admin.mmails.create')) && (!$trash))
        @include('mixdinternet/admix::partials.actions.btn.insert', ['route' => route('admin.mmails.create')])
    @endif
    @if((!checkRule('admin.mmails.trash')) && (!$trash))
        @include('mixdinternet/admix::partials.actions.btn.trash', ['route' => route('admin.mmails.trash')])
    @endif
    @if($trash)
        @include('mixdinternet/admix::partials.actions.btn.list', ['route' => route('admin.mmails.index')])
    @endif
@endsection

@section('btn-delete-all')
    @if((!checkRule('admin.mmails.destroy')) && (!$trash))
        @include('mixdinternet/admix::partials.actions.btn.delete-all', ['route' => route('admin.mmails.destroy')])
    @endif
@endsection
        ...
        <table class="table table-striped table-hover table-action jq-table-rocket">
            ...
            @foreach ($mmails as $mmail)
                <tr>
                    @if((!checkRule('admin.mmails.destroy')) && (!$trash))
                        <td>
                            @include('mixdinternet/admix::partials.actions.checkbox', ['row' => $mmail])
                        </td>
                    @endif
                    <td>{{ $mmail->id }}</td>
                    <td>{{ $mmail->name }}</td>
                    <td>{{ $mmail->slug }}</td>
                    <td>@include('mixdinternet/admix::partials.label.status', ['status' => $mmail->status])</td>
                    <td>
                        @if((!checkRule('admin.mmails.edit')) && (!$trash))
                            @include('mixdinternet/admix::partials.actions.btn.edit', ['route' => route('admin.mmails.edit', $mmail->id)])
                        @endif
                        @if((!checkRule('admin.mmails.destroy')) && (!$trash))
                            @include('mixdinternet/admix::partials.actions.btn.delete', ['route' => route('admin.mmails.destroy'), 'id' => $mmail->id])
                        @endif
                        @if($trash)
                            @include('mixdinternet/admix::partials.actions.btn.restore', ['route' => route('admin.mmails.restore', $mmail->id)])
                        @endif
                    </td>
                </tr>
            @endforeach
...
@section('pagination-showing')
    @include('mixdinternet/admix::partials.pagination-showing', ['model' => $mmails])
@endsection